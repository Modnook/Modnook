...
func (_q *ProjectQuery) QueryGame() *GameQuery {
...
func (_q *ProjectQuery) QueryProjectType() *ProjectTypeQuery {
...
func (_q *ProjectQuery) WithGame(opts ...func(*GameQuery)) *ProjectQuery {
	query := (&GameClient{config: _q.config}).Query()
...
func (_q *ProjectQuery) WithProjectType(opts ...func(*ProjectTypeQuery)) *ProjectQuery {
	query := (&ProjectTypeClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	...
	if query := _q.withGame; query != nil {
		if err := _q.loadGame(ctx, query, nodes, nil,
			func(n *Project, e *Game) { n.Edges.Game = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *ProjectQuery) loadGame(ctx context.Context, query *GameQuery, nodes []*Project, init func(*Project), assign func(*Project, *Game)) error {
...
func (_q *ProjectQuery) loadProjectType(ctx context.Context, query *ProjectTypeQuery, nodes []*Project, init func(*Project), assign func(*Project, *ProjectType)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Project)
	for i := range nodes {
...
func (_q *ProjectQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := _q.ctx.Fields; len(fields) > 0 {
		...
		if _q.withGame != nil {
			_spec.Node.AddColumnOnce(project.FieldGameID)
		}
		if _q.withProjectType != nil {
			_spec.Node.AddColumnOnce(project.FieldProjectTypeID)
		}
	}
