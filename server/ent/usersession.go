...
func (*UserSession) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case usersession.FieldSessionToken, usersession.FieldIPAddress, usersession.FieldUserAgent, usersession.FieldExpiresAt, usersession.FieldLastAccessedAt:
	...
	for i := range columns {
		...
		case usersession.FieldLastAccessedAt:
			if value, ok := values[i].(*sql.NullString); !ok {
				...
				_m.LastAccessedAt = new(string)
				*_m.LastAccessedAt = value.String
			}
